...
def main():
    ...
    for name, item in pytesseract_dict.items():
        result, is_blank = load_and_generate(model, tokenizer, device, f"{base_prompt}: {item}")
        print("did one")
        if is_blank:
            blank_count += 1
        name = name.rstrip(".txt")
        name = name.lstrip("pytesseract")
        bart_name = args.checkpoint.split("work/")[-1]
        entry = name + "corrected_by_bart" + bart_name
